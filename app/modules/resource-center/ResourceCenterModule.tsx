...
export default function ResourceCenterModule({ units, subjectName, initialSection = "curriculum", onOpen }: { units: Unit[]; subjectName: string; initialSection?: ResourceSection; onOpen:(view:AppView)=>void }) {
  ...
  return <section className="resource-center" id="top">
    <header className="resource-hero"><div><span className="eyebrow"><BookOpen size={15}/> FOPOS • TYMM Kaynak Merkezi</span><h1>Doğrulanmış müfredatı<br/><em>tek ekranda inceleyin.</em></h1><p>{[...new Set(units.map((unit) => unit.grade))].join(" ve ")}. sınıf {subjectName.toLocaleLowerCase("tr-TR")} ünitelerini ve öğrenme çıktılarını planlama öncesinde inceleyin.</p></div><div className="resource-verification"><ShieldCheck size={24}/><strong>Doğrulanmış veri katmanı</strong><span>{subjectName} Dersi • {[...new Set(units.map((unit) => unit.grade))].join("–")}. Sınıf</span></div></header>
    ...
    <div className={`resource-toolbar ${section !== "curriculum" ? "bep-toolbar" : ""}`} role="search">{section === "curriculum" && <label><span>Sınıf</span><select value={grade} onChange={(event) => setGrade(event.target.value === "all" ? "all" : Number(event.target.value) as Grade)}><option value="all">Tüm sınıflar</option>{[...new Set(units.map((unit) => unit.grade))].sort((left, right) => left - right).map((item) => <option value={item} key={item}>{item}. Sınıf</option>)}</select></label>}<label className="resource-search"><span>{section === "curriculum" ? "Ünite, kod, kavram veya öğrenme çıktısı ara" : section === "bep" ? "İhtiyaç alanı veya uyarlama ara" : "Belge türü, bölüm veya kullanım amacı ara"}</span><div><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={section === "curriculum" ? "Ünite kodu, kavram veya öğrenme çıktısı…" : section === "bep" ? "Örn. okuma, ek süre, görsel erişim…" : "Örn. yıllık plan, belirtke, karar…"}/></div></label></div>
