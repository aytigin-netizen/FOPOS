...
function nextBranch(
  ...
  const used = new Set(
    workspaces
      .filter(
        (item) => item.subjectCode === subjectCode && item.grade === grade,
      )
  ...
  async function create(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/class-workspaces", {
        ...
        body: JSON.stringify({ subjectCode, grade, branchCode }),
      });
      ...
      setWorkspaces(payload.workspaces);
      setBranchCode(nextBranch(payload.workspaces, subjectCode, grade));
      ...
      const disciplineName =
        disciplines.find((discipline) => discipline.code === subjectCode)?.name ??
        subjectCode;
      setMessage(
        disciplineName +
          " • " +
          grade +
          "/" +
          branchCode.trim().toLocaleUpperCase("tr-TR") +
