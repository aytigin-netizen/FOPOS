...
export default function Home() {
  ...
  const [archiveNeedsReset, setArchiveNeedsReset] = useState(false);
  const [archiveResetConfirmed, setArchiveResetConfirmed] = useState(false);
  ...
  function storeResult(next: PlanResult) {
    try {
      saveRecordRevision(window.localStorage, next.pedagogicalRecord);
      setRecordSaveStatus(
        `Yerel arşive kaydedildi • Revizyon ${next.pedagogicalRecord.revision}`,
      );
      setRecordHistory(
        listRecordRevisions(
          window.localStorage,
          next.pedagogicalRecord.recordId,
        ),
      );
      setArchiveNeedsReset(false);
    } catch (error) {
      setRecordSaveStatus(
        error instanceof Error ? error.message : "Kayıt saklanamadı",
      );
      const archiveStatus = inspectRecordArchive(window.localStorage);
      setArchiveNeedsReset(
        ["corrupt", "unsupported", "oversized"].includes(archiveStatus.state),
      );
    }
    setResult(next);
  }

  function clearInvalidArchive() {
    try {
      clearRecordArchive(window.localStorage, archiveResetConfirmed);
      setArchiveNeedsReset(false);
      setArchiveResetConfirmed(false);
      setRecordHistory([]);
      setRecordSaveStatus("Sorunlu yerel arşiv öğretmen onayıyla temizlendi");
    } catch (error) {
      setRecordSaveStatus(
        error instanceof Error ? error.message : "Yerel arşiv temizlenemedi",
      );
    }
  }
  ...
  async function generatePlan() {
    ...
    try {
      ...
      storeResult(makeResult(selectedUnit, outcome, profile, week));
      setProgress(100);
            ...
            <section className="results-section" ref={resultsRef} tabIndex={-1}>
              <div className="results-header">
                <div>
                  ...
                  {archiveNeedsReset ? (
                    <div className="archive-recovery" role="alert">
                      <strong>Yerel arşiv kullanılamıyor</strong>
                      <span>
                        Arşiv otomatik silinmedi. Temizleme yalnız aşağıdaki
                        açık öğretmen onayıyla yapılır; mevcut yerel revizyonlar
                        geri alınamaz.
                      </span>
                      <label>
                        <input
                          type="checkbox"
                          checked={archiveResetConfirmed}
                          onChange={(event) =>
                            setArchiveResetConfirmed(event.target.checked)
                          }
                        />
                        Sorunlu yerel arşivin silineceğini anlıyorum
                      </label>
                      <button
                        type="button"
                        className="secondary-button"
                        disabled={!archiveResetConfirmed}
                        onClick={clearInvalidArchive}
                      >
                        Sorunlu yerel arşivi temizle
                      </button>
                    </div>
                  ) : null}
                </div>
                <div className="result-actions">
                  <button className="secondary-button" onClick={createRevision}>
