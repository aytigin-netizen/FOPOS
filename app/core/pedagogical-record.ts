...
export type PedagogicalRecord={schemaVersion:"1.0.0";recordId:string;revision:number;status:RecordStatus;createdAt:string;updatedAt:string;previousRevision:number|null;approval:Approval|null;curriculum:{datasetVersion:string;grade:Grade;unitCode:string;outcomeCode:string};lessonContext:{week:number;durationMinutes:number;profile:string};pedagogicalDecision:{strategy:string;methods:string[];learningEvidence:string}};
...
export function createPedagogicalRecord(input:{unit:Unit;outcomeCode:string;week:number;profile:string;datasetVersion:string;durationMinutes?:number}):PedagogicalRecord{
  ...
  return{schemaVersion:"1.0.0",recordId:makeId("OPUS-PR"),revision:1,status:"draft",createdAt:now,updatedAt:now,previousRevision:null,approval:null,curriculum:{datasetVersion:input.datasetVersion,grade:input.unit.grade,unitCode:input.unit.code,outcomeCode:input.outcomeCode},lessonContext:{week:input.week,durationMinutes:input.durationMinutes??80,profile:input.profile},pedagogicalDecision:{strategy:input.unit.strategy,methods:[...input.unit.methods],learningEvidence:input.unit.evidence}};
}
